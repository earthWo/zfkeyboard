...
public class PopKeyboardView extends View implements View.OnClickListener {
    ...
    private Drawable mKeyBroadBackground;
    ...
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public PopKeyboardView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setFontSize(String mode){
        if("1".equals(mode)){
            //大
            mKeyTextSize=normalFontSize+3;
        }else if("2".equals(mode)){
            //小
            mKeyTextSize=normalFontSize-3;
        }else{
            mKeyTextSize=normalFontSize;
        }
    }
    ...
    public void setSkin(String skin){
        if("1".equals(skin)){
            //黑色
            mKeyBroadBackground=new ColorDrawable(Color.BLACK);
        }else if("2".equals(skin)){
            //小
            mKeyBroadBackground=new ColorDrawable(Color.RED);
        }else{
            mKeyBroadBackground=new ColorDrawable(ContextCompat.getColor(getContext(),R.color.background));
        }
    }


    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public PopKeyboardView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);

        TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.ZFKeyboardView, defStyleAttr, defStyleRes);
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.ZFKeyboardView_keyBackground) {
...
//
//            }
            else if (attr == R.styleable.ZFKeyboardView_keyPreviewLayout) {
...
//
//            }
            else if (attr == R.styleable.ZFKeyboardView_keyPreviewHeight) {
                mPreviewHeight = a.getDimensionPixelSize(attr, 80);
            } else if (attr == R.styleable.ZFKeyboardView_keyTextSize) {
                mKeyTextSize = a.getDimensionPixelSize(attr, 18);

            } else if (attr == R.styleable.ZFKeyboardView_KeyTextColor) {
                mKeyTextColor = a.getColor(attr, 0xFF000000);

            } else if (attr == R.styleable.ZFKeyboardView_labelTextSize) {
                mLabelTextSize = a.getDimensionPixelSize(attr, 14);

            } else if (attr == R.styleable.ZFKeyboardView_popupLayout) {
                mPopupLayout = a.getResourceId(attr, 0);

            } else if (attr == R.styleable.ZFKeyboardView_shadowColor) {
                mShadowColor = a.getColor(attr, 0);

            } else if (attr == R.styleable.ZFKeyboardView_shadowRadius) {
                mShadowRadius = a.getFloat(attr, 0f);

            }
            else if (attr == R.styleable.ZFKeyboardView_keyBroadBackground) {
                mKeyBroadBackground = a.getDrawable(attr);
            }
        }
     ...
     * version of the depressed key. By default the layout_key_preview is enabled.
    ...
    /**
     * When enabled, calls to  will include key
...
//            mKeyboard.resize(w, h);
    ...
    private void onBufferDraw() {
        ...
        mKeyBroadBackground.setBounds(0, 0,getMeasuredWidth(), getMeasuredHeight());
//        mKeyBroadBackground.draw(canvas);
        ...
        for (int i = 0; i < keyCount; i++) {
            ...
            if (label != null) {
                ...
                if(key.popupCharacters!=null){
                    top= (int) (key.height*.08);
                    String popupChar = key.popupCharacters.toString();
                    paint.setTextSize(textSize/2);
                    canvas.drawText(popupChar,
                            (key.width-textSize/2) ,
                            key.height/4,
                            paint);
                }
                ...
                canvas.drawText(label,
                        ...
                        (key.height - padding.top - padding.bottom) / 2
                                + (paint.getTextSize() - paint.descent()) / 2 + padding.top+top,
                        paint);
    ...
    public int[] getNearestKeys(int x, int y) {
        int [][]mGridNeighbors = new int[50][];
        int keyHeight= mKeyboard.getHeight()/(1+mKeyboard.getKeys().size()/3);
        int size=mKeyboard.getKeys().size()>3?3:mKeyboard.getKeys().size();
        int keyWidth=mKeyboard.getMinWidth()/size;
        if (x >= 0 && x < mKeyboard.getMinWidth() && y >= 0 && y < mKeyboard.getHeight()) {
            int index = (y /keyHeight) * 3 + (x / keyWidth);
            if (index < 50) {
    ...
    private void showPreview(int keyIndex) {
        ...
        // If key changed and layout_key_preview is on ...
...
//            mPopupPreviewY = key.y - popupHeight + mPreviewOffset;
        ...
        // Set the layout_key_preview background state
...
//                key.popupResId != 0 ? LONG_PRESSABLE_STATE_SET : EMPTY_STATE_SET);
    ...
    public boolean onTouchEvent(MotionEvent me) {
        ...
        if(me.getAction()==MotionEvent.ACTION_CANCEL&&mMiniKeyboardOnScreen){
